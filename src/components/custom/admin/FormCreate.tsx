...
export function FormCreate() {
  ...
  const handleQuestionTypeChange = (index: number) => (value: string) => {
    const newQuestions = form.questions.map((question, qIndex) => {
      if (qIndex === index) {
        return { ...question, type: value, possibleAnswers: value === "MULTIPLE_CHOICE" ? [""] : null };
      }
      return question;
    });
    setForm({ ...form, questions: newQuestions });
  ...
  const handleOptionChange = (questionIndex: number, optionIndex: number, value: string) => {
    const newQuestions = form.questions.map((question, qIndex) => {
      if (qIndex === questionIndex) {
        const newPossibleAnswers = question.possibleAnswers?.map((option, oIndex) => {
  ...
  const handleRemoveOption = (questionIndex: number, optionIndex: number) => {
    const newQuestions = form.questions.map((question, qIndex) => {
      if (qIndex === questionIndex) {
        const newPossibleAnswers = question.possibleAnswers?.filter((_, oIndex) => oIndex !== optionIndex);
        return { ...question, possibleAnswers: newPossibleAnswers };
      }
      return question;
    })as QuestionDTO[];
  ...
  return(
    <Card className="w-full h-full rounded-none">
      <form id="form-creation-form" onSubmit={handleFormSave}>
        <CardHeader>
          <CardTitle>
            <Input placeholder="Form Title" className="placeholder:text-5xl text-5xl h-20" value={form.name} onChange={handleTitleChange} required/> 
          </CardTitle>
            <Textarea placeholder="Form Description" value={form.description} onChange={handleDescriptionChange}/> 
            <div className="text-sm text-muted-foreground mt-16 flex flex-row">
              <fieldset className="flex items-center space-x-2 m-2">
                <Checkbox id="terms" checked={form.published} onCheckedChange={handleChangePublished}/>
                <label
                  htmlFor="terms"
          ...
          <Accordion type="multiple" className="w-full">
            {
              form.questions.map(
                (question,questionIndex) => 
                  <AccordionItem key={`form-question-${questionIndex}`} value={`form-question-${questionIndex}`}>
                    <AccordionTrigger>
                      <Input 
                    ...
                    <AccordionContent className="ml-2">
                      ...
                      <div className="h-2"/>
                      <Select onValueChange={handleQuestionTypeChange(questionIndex)} defaultValue={question.type} required>
                        ...
                        <>
                          ...
                          <div className="flex flex-col gap-2 mt-2">
                          {
                            question.possibleAnswers?.map(
                              (option, index) => (
                                <div key={`question-possible-answer-${index}`} className="flex items-center gap-2">
                                  <Input
                                    value={option}
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => {handleRemoveOption(questionIndex,index)}}
                                    type="button"
                                  > 
                                  {
                                    index !== 0 && <X/>
                          ...
                          <Button variant="outline" onClick={() => {handleAddOption(questionIndex)}} className="mt-4" type="button">
                            Add Choice
                          </Button>
                        </>
                      }
        ...
        <CardFooter className="flex flex-row gap-2">
          <Button onClick={handleAddQuestion} type="button" >Add</Button>
